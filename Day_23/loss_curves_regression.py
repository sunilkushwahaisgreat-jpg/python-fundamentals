...
for i in range(itr):
    ...
    bias=bias-learning_rate*db
    current_loss=np.mean(np.square(y_actual-y_pred))
    loss_minmax.append(current_loss)
...
plt.plot(loss_base, label="Baseline (No Scaling)")
plt.plot(loss_minmax, label="Min-Max Normalized")
plt.plot(loss_std, label="Standardized")
